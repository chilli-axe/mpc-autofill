...
export function base_on_load() {
  ...
  document
    .getElementById("cookieToast")
    .addEventListener("show.bs.toast", cookie_toast_shown); // TODO: should we use shown.bs.toast instead?
  if (Cookies.get("ga_disabled") === undefined) {
    cookie_toast.show();
  }
  ...
  document
    ...
    .addEventListener("hide.bs.toast", error_toast_hidden);
  document
    .getElementById("errorToast")
    .addEventListener("show.bs.toast", error_toast_shown);
...
export function handle_error(exc) {
  ...
  if (exc !== "" && exc !== undefined && exc !== null) {
    ...
    document.getElementById("error_message_paragraph").textContent =
      "Sorry about that! If the issue persists, please let us know on Reddit or Discord.";
  }
  error_toast.show();
}
