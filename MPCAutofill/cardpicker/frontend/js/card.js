...
class CardBase {
  ...
  open_detailed_view() {
    ...
    if (
      curr_img.download_link === undefined ||
      curr_img.download_link === null
    ) {
...
export class Card extends CardBase {
  ...
  setup_card(cards, query, dom_id, slot, face, req_type, group, selected_img) {
    ...
    this.elem_img_prev = document.getElementById(
      this.dom_id + "-card-img-prev"
    );
    this.elem_img_next = document.getElementById(
      this.dom_id + "-card-img-next"
    );
    ...
    this.card_counter = document.getElementById(
      this.dom_id + "-mpccard-counter"
    );
    this.card_counter_btn = document.getElementById(
      this.dom_id + "-mpccard-counter-btn"
    );
  ...
  load_thumbnails() {
    if (!this.empty) {
      ...
      this.elem_img.src =
        this.cards[wrap0(this.img_idx, this.img_count)].small_thumbnail_url;

      // respect the length of the returned results
      if (this.img_count > 1) {
        this.elem_img_prev.src =
          this.cards[
            wrap0(this.img_idx - 1, this.img_count)
          ].small_thumbnail_url;
  ...
  search_in_place(search_query) {
    ...
    $(this.pe).animate(
      ...
      function () {
        ...
        let common_back_id = "";
        if (
          (search_query_trimmed === "" || search_query_trimmed === null) &&
          this.face === "back"
        ) {
          search_type = "back";
          // specify common cardback ID
          common_back_id = $("#slot--back")
            .data("obj")
            .get_curr_img().identifier;
        }
  ...
  update_card() {
    ...
    if (!this.empty) {
      ...
      curr_source = format_source(
        this.get_curr_img().source_verbose,
        this.get_curr_img().dpi
      );
    }
...
export class CardRecent extends CardBase {
  ...
  setup_card(cards, query, dom_id, slot, face, req_type, group, selected_img) {
    ...
    this.elem_source.innerText = format_source(
      cards[0].source_verbose,
      cards[0].dpi
    );
